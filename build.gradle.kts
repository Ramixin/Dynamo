...
dependencies {
    ...
    commonImplementation(files("libs/Stator-1.0.0-universal.jar"))

    commonAnnotationProcessor(files("libs/Stator-1.0.0-universal.jar"))
    commonAnnotationProcessor("com.google.code.gson:gson:2.13.1")
}
...
manifests {
    ...
    fabricModJson(sourceSets.fabric.get()) {
        ...
        entrypoint("client", "$group.$projectModId.fabric.${prop("main_class")}")
        entrypoint("server", "$group.$projectModId.fabric.${prop("main_class")}")
